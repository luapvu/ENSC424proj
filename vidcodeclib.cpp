    ...
    int x)
{
    int i, j, iSAD0, iSAD;
    int iMVy = 0, iMVx = 0;
	int step_size_lm = 4; // Large motion
	int step_size_sm = 1; // Small motion
	bool small_motion = false;

    iSAD0 = GetSAD(pfCurrFrame, pfRefFrame, y, x, 0, 0, 65535);
    
	// Large motion
    for (i = -step_size_lm; i <= step_size_lm; i=i+step_size_lm) {
       for (j = -step_size_lm; j <= step_size_lm; j=j+step_size_lm) {
		   ...
		   // We want to check the 0,0 location so comment out this
           //if (i == 0 && j == 0) {
           //    continue;
           //}
    ...
    for (i = -step_size_sm; i <= step_size_sm; i=i+step_size_sm) {
       for (j = -step_size_sm; j <= step_size_sm; j=j+step_size_sm) {
		   ...
		   // We already checked 0,0 location, no need to check again
           if (i == 0 && j == 0) {
               continue;
           }
	...
	// Finish small motion algorithm

	if (small_motion){
		for (i = iMVy-step_size_sm; i <= iMVy+step_size_sm; i=i+step_size_sm) {
		   for (j = iMVx-step_size_sm; j <= iMVx+step_size_sm; j=j+step_size_sm) {
	...
	// Finish large motion algorithm
		...
		while (step_size_lm >= 1){
			...
			for (i = iMVy-step_size_lm; i <= iMVy+step_size_lm; i=i+step_size_lm) {
			   for (j = iMVx-step_size_lm; j <= iMVx+step_size_lm; j=j+step_size_lm) {
